...
export class Driver extends DriverNoVendor {
    ...
    public static async connect(net: Net, wallet?: Wallet) {
        const genesis: Connex.Meter.Block = await net.http('GET', 'blocks/0')
        const best: Connex.Meter.Block = await net.http('GET', 'blocks/best', {
    ...
    private async estimateGas(
        ...
        caller: string) {
        const outputs: Connex.Meter.VMOutput[] = await this.explain({
            clauses,
            caller,
        }, this.head.id)
