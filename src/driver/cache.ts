...
export class Cache {
  ...
  private auctionSummaries: Flex.Meter.AuctionSummary[] = [];
  ...
  public handleNewBlock(
    ...
    this.window.push({
      ...head,
      bloom: bloom
        ? new Bloom(bloom.k, Buffer.from(bloom.bits.slice(2), 'hex'))
        : undefined,
      block,
      accounts: new Map(),
      txs: new Map(),
  ...
  public async getCandidates(fetch: () => Promise<Flex.Meter.Candidate[]>) {
    let currHeight = 0;
  ...
  public async getStakeholders(fetch: () => Promise<Flex.Meter.Stakeholder[]>) {
    let currHeight = 0;
    if (this.window.length > 0) {
      const top = this.window[this.window.length - 1];
      currHeight = top.number;
  ...
  public async getAuction(fetch: () => Promise<Flex.Meter.Auction>) {
    let currHeight = 0;
    if (this.window.length > 0) {
      const top = this.window[this.window.length - 1];
      currHeight = top.number;
...
class Account {
  constructor(
    readonly obj: Flex.Meter.Account,
